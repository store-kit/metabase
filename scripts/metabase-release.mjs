...
// Whether this bump goes through a PR (opened and merged automatically —
// no human review step) rather than committing straight to master. Both
// paths are fully automatic; the difference is that a "major" bump (a new
// feature line that can ship an irreversible app-DB migration) leaves a
// reviewable PR/diff behind for later reference, while a "minor" bump
// (same feature line, bug/security fixes only) has no such need and just
// commits directly. Single source of truth for the workflow's branching,
// so the policy lives in one tested place instead of being duplicated
// across multiple `if:` conditions in the YAML.
export function usesPullRequestFlow(kind) {
    return kind === 'major';
}
